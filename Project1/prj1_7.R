...
for (i in 7:7){ #1:length(nodes_id)){ #length(nodes_id)
...
#CHECK: should this be directed?
    ...
    #creating subgraph from each community
    ...
    #check which circle each node belongs to
    ...
    for (t in 1:length(V(g_sub_1))){
      ...
      for (j in 1:length(circles[[i]])){
        for(k in 1:length(circles[[i]][[j]])){
          if(V(g_sub_1)[t]$name == circles[[i]][[j]][[k]]){
            print("t")
            print(t)
            print("true")
            count <- count + 1
    ...
    #Plot overlaps between community and circle 
